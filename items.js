...
function ItemDAO(database) {
    ...
    this.db = database;
    let item = this.db.collection('item');
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        item.aggregate([ {"$group": { "_id":  "$category", "num": { "$sum" : 1 } } }, { $sort: { "_id": 1 } } ], (error,result) => {
            result.forEach((items) => {
                categories.push(items);
            });
            item.aggregate([ { $count: "category" } ], (error,result) => {
                let total = result[0].category;
